...
function FormularioTema() {
    ...
    const [tipo, setTipo] = useState<Tipo>({} as Tipo)  // Iniciamos um objeto vazio da Model Tema
    ...
    async function buscarPorId(id: string) {
        ...
        await buscar(`/temas/${id}`, setTipo, { // Aqui, na URL de Requisição, passamos o ID do Tema a ser Buscado
            headers: {
    ...
    function atualizarEstado(e: ChangeEvent<HTMLInputElement>) {
        ...
        setTipo({       // atualiza o objeto/estado Tema com os dados digitados no input
            ...tipo,    //  o spread operator (...) espalha os atributos do objeto para facilitar a atualização
            ...
            <form className="w-1/2 flex flex-col gap-4" onSubmit={gerarNovoTipo}>
                <div className="flex flex-col gap-2">
                    ...
                    <input
                        ...
                        value={tipo.descricao}                                                  // Conecta esse input com o atributo nome do estado/objeto tema
                        onChange={(e: ChangeEvent<HTMLInputElement>) => atualizarEstado(e)}     // Quando o usuario digitar algo, chama a função atualizarEstado
                    />
                </div>
                <div className="flex flex-col gap-2">
                    <label htmlFor="nome">Descrição do tema</label>
                    <input
                        type="text"
                        placeholder="Nome"
                        name='nome'
                        className="border-2 border-slate-700 rounded p-2"
                        value={tipo.nome}                                                  // Conecta esse input com o atributo nome do estado/objeto tema
                        onChange={(e: ChangeEvent<HTMLInputElement>) => atualizarEstado(e)}     // Quando o usuario digitar algo, chama a função atualizarEstado
                    />
                </div>
                <div className="flex flex-col gap-2">
                    <label htmlFor="data">Descrição do tema</label>
                    <input
                        type="date"
                        placeholder="Data"
                        name='data'
                        className="border-2 border-slate-700 rounded p-2"
                        value={tipo.data}                                                  // Conecta esse input com o atributo nome do estado/objeto tema
                        onChange={(e: ChangeEvent<HTMLInputElement>) => atualizarEstado(e)}     // Quando o usuario digitar algo, chama a função atualizarEstado
                    />
                </div>
                <button
